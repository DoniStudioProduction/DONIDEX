...
export default async (request: Request) => {
  ...
  const payload = { email: identity.email, plan: body.priceId, callback_url: `${origin}/?billing=paystack-return`, metadata: JSON.stringify({ userId: identity.uid, email: identity.email, planCode: body.priceId }) };
