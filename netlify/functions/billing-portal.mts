...
export default async (request: Request) => {
  ...
  if (!body?.subscriptionCode) return Response.json({ error: 'A Paystack subscription code is required.' }, { status: 400 });
  const response = await fetch(`https://api.paystack.co/subscription/${encodeURIComponent(body.subscriptionCode)}/manage/link`, { headers: { Authorization: `Bearer ${secret}` } });
