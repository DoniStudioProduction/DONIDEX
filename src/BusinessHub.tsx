...
export default function BusinessHub() {
 ...
 return <>
  ...
  {open && <div className="hub-backdrop" onClick={() => setOpen(false)}><section className="hub-modal" onClick={e => e.stopPropagation()}>
   <header className="hub-header"><div><p className="hub-eyebrow">DONIDEX BUSINESS HUB</p><h2>Growth & automation</h2><p>Turn everyday business activity into follow-up, recurring revenue and clearer decisions.</p>{!canEdit && <small className="muted">Staff access is read-only.</small>}</div><button className="hub-close" onClick={() => setOpen(false)} aria-label="Close Business Hub"><X size={18}/></button></header>
   ...
   {tab === 'Products' && <div className="hub-content"><div className="hub-section-head"><div><strong>Products & services</strong><span>Create a reusable catalogue for faster invoicing.</span></div>{canEdit && <button className="hub-primary" onClick={addProduct}><Plus size={15}/> Add item</button>}</div>{products.length === 0 ? <div className="hub-empty"><Package size={30}/><strong>No catalogue items yet</strong><span>{canEdit ? 'Add your first product or service and it will stay available for future documents.' : 'No catalogue items have been created yet.'}</span>{canEdit && <button className="hub-primary" onClick={addProduct}>Create first item</button>}</div> : <div className="hub-list">{products.map(p => <div className="hub-row" key={p.id}><div><strong>{p.name}</strong><span>{p.type}</span></div><b>{money(p.price)}</b><ChevronRight size={16}/></div>)}</div>}</div>}
   {tab === 'Recurring' && <div className="hub-content"><div className="hub-section-head"><div><strong>Recurring invoices</strong><span>Prepare predictable monthly or yearly billing.</span></div>{canEdit && <button className="hub-primary" onClick={addRecurring}><Plus size={15}/> Add profile</button>}</div>{recurring.length === 0 ? <div className="hub-empty"><CalendarClock size={30}/><strong>No recurring profiles yet</strong><span>{canEdit ? 'Set up a customer, amount and billing interval for repeat revenue.' : 'No recurring profiles have been created yet.'}</span>{canEdit && <button className="hub-primary" onClick={addRecurring}>Create first profile</button>}</div> : <div className="hub-list">{recurring.map(r => <div className="hub-row" key={r.id}><div><strong>{r.customer}</strong><span>{r.interval} · Next {r.next}</span></div><b>{money(r.amount)}</b><ChevronRight size={16}/></div>)}</div>}</div>}
   ...
   <footer className="hub-footer">Business Hub changes are included in the authenticated workspace sync. Staff members remain read-only.</footer>
  </section></div>}
 </>;
}
