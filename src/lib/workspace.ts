...
export type DonidexWorkspace = { profile?: Record<string, unknown>; businesses?: DonidexBusiness[]; activeBusinessId?: string; documents?: Record<string, unknown>[]; customers?: Record<string, unknown>[]; expenses?: Record<string, unknown>[]; settings?: Record<string, unknown>; hubProducts?: Record<string, unknown>[]; hubRecurring?: Record<string, unknown>[]; migratedAt?: number; updatedAt?: number };
...
export function workspaceFromLocalStorage(): DonidexWorkspace {
  ...
  return { profile: read('profile'), businesses: business ? [{ ...business, id: businessId, archived: false }] : [{ id: businessId, name: 'My Business', archived: false, createdAt: Date.now() }], activeBusinessId: businessId, documents: read('docs') || [], customers: read('customers') || [], expenses: read('expenses') || [], settings: read('settings'), hubProducts: read('hub_products') || read('products') || [], hubRecurring: read('hub_recurring') || read('recurring') || [], migratedAt: Date.now() };
}
...
export function applyWorkspaceToLocalStorage(workspace: DonidexWorkspace) {
  ...
  write('profile', workspace.profile || {}); write('docs', workspace.documents || []); write('customers', workspace.customers || []); write('expenses', workspace.expenses || []); if (active) write('business', active); if (workspace.settings) write('settings', workspace.settings); write('hub_products', workspace.hubProducts || []); write('hub_recurring', workspace.hubRecurring || []); write('products', workspace.hubProducts || []); write('recurring', workspace.hubRecurring || []);
}
